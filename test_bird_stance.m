...
parms.speed=1; % [m/s] desired walking speed
...
x0 = [.5*pi+parms.alpha; 0; 1*parms.L; 0]; % [phi phid xh xhd] a sample initial state
...
for iStep=1:length(t)
    ...
    plot([0 r_p(1)],[0 r_p(2)],'k',xh(iStep),yh,'ro'); hold on
    drawnow
    axis equal
end
